...
class UuidValidator implements ValidatorInterface
{
    ...
    {
        return $object instanceof JsonSchema && ((\is_array($object->getType()) ? \in_array('string', $object->getType(), true) : 'string' === $object->getType()) || null === $object->getType()) && 'uuid' === $object->getFormat();
    }

    public function guess(mixed $object, string $name, ClassGuess|Property $guess): void
    {
        $guess->addValidatorGuess(new ValidatorGuess(Uuid::class));
    }
}
