...
class MaximumValidator implements ValidatorInterface
{
    ...
    {
        return $object instanceof JsonSchema && (\is_array($object->getType()) ? (\in_array('integer', $object->getType(), true) || \in_array('number', $object->getType(), true)) : ('integer' === $object->getType() || 'number' === $object->getType())) && is_numeric($object->getMaximum());
    }
    ...
    public function guess(mixed $object, string $name, ClassGuess|Property $guess): void
    {
        ...
        $guess->addValidatorGuess(new ValidatorGuess(LessThanOrEqual::class, [
            'value' => $object->getMaximum(),
        ]));
    }
}
