...
class MultipleOfValidator implements ValidatorInterface
{
    ...
    {
        return $object instanceof JsonSchema && (\is_array($object->getType()) ? (\in_array('integer', $object->getType(), true) || \in_array('number', $object->getType(), true)) : ('integer' === $object->getType() || 'number' === $object->getType())) && is_numeric($object->getMultipleOf()) && ((int)$object->getMultipleOf()) > 0;
    }
