<?php

/*
 * This file is auto-generated by PHP OpenAPI Generator.
 * DO NOT EDIT MANUALLY — changes will be overwritten when the generator is re-run.
 */
declare (strict_types=1);
namespace LongTermSupport\OpenApiGenerator\Component\OpenApi3\Tests\Expected\Github\Validator;

class CombinedBillingUsageConstraint extends \Symfony\Component\Validator\Constraints\Compound
{
    protected function getConstraints($options): array
    {
        return [new \Symfony\Component\Validator\Constraints\Collection(fields: ['days_left_in_billing_cycle' => new \Symfony\Component\Validator\Constraints\Optional([]), 'estimated_paid_storage_for_month' => new \Symfony\Component\Validator\Constraints\Optional([]), 'estimated_storage_for_month' => new \Symfony\Component\Validator\Constraints\Optional([])], allowExtraFields: true)];
    }
}